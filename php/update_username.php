        ...
        try {
            ...
            if (isset($input['user_name']) && isset($input['password'])) {
                ...
                if ($result->num_rows > 0) {
                    $sqlsorgu = $baglanti->prepare("UPDATE kullanicilar SET user_name = ? WHERE id = ?");
                    $sqlsorgu->bind_param("si", $user_name, $user_id);
                    if ($sqlsorgu->execute()) {
                        $response["success"] = 1;
                        $response["message"] = "Kullanıcı adı başarıyla güncellendi";
                    } else {
