...
if (isset($headers['Authorization'])) {
    ...
    if ($jwt) {
        try {
            $key = "your_secret_key";
            $decoded = JWT::decode($jwt, new \Firebase\JWT\Key($key, 'HS256')); // Güncellenmiş parametre çağrısı
            ...
            // Todos öğelerini getirme ve kategorilere ekleme
            $todoQuery = $baglanti->prepare("SELECT * FROM todos WHERE user_id = ? ORDER BY created_date DESC");
