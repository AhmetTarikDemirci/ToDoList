...
if (isset($headers['Authorization'])) {
    ...
    if ($jwt) {
        try {
            ...
            // Todos öğelerini getirme ve kategorilere ekleme (is_done = true)
            $todoQuery = $baglanti->prepare("SELECT * FROM todos WHERE user_id = ? AND is_done = 1 ORDER BY created_date DESC");
            ...
            // Boş olmayan kategorileri filtreleme
            $filteredCategories = array_filter($categories, function($category) {
                return !empty($category["todos"]);
            });

            $response = array_values($filteredCategories);
